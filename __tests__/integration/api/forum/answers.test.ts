...
import {
  POST as votePost,
  DELETE as voteDelete,
} from '@/app/api/forum/answers/[id]/vote/route'
...
describe('Forum Answers Route', () => {
  ...
  beforeEach(() => {
    ...
    applySupabaseMock(createClient as jest.Mock, {
      tableResults: {
        forum_questions: {
          data: { id: 'q-1', author_id: 'investor-99', answer_count: 0 },
        },
        forum_answers: {
          data: {
            id: 'answer-1',
            content: 'Start with index funds',
            author_id: 'advisor-123',
            votes_count: 5,
  ...
  describe('POST /api/forum/questions/[id]/answers', () => {
    it('should return 401 when no access token', async () => {
      ...
      const request = new NextRequest(
        ...
        { method: 'POST', body: JSON.stringify({ content: 'My answer here' }) }
      )
      const response = await answerPost(request, { params: { id: 'q-1' } })
    ...
    it('should increment answer_count on question', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/forum/questions/q-1/answers',
        { method: 'POST', body: JSON.stringify({ content: 'Here is my answer' }) }
      )
      const response = await answerPost(request, { params: { id: 'q-1' } })
  ...
  describe('POST /api/forum/answers/[id]/vote', () => {
    it('should return 401 when no access token', async () => {
      ...
      const response = await votePost(request, { params: { id: 'answer-1' } })
      expect(response.status).toBe(401)
    })
    ...
    it('should create helpful vote', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/forum/answers/answer-1/vote',
        { method: 'POST', body: JSON.stringify({ vote_type: 'helpful' }) }
      )
      const response = await votePost(request, { params: { id: 'answer-1' } })
      expect(response.status).toBe(200)
      const data = await response.json()
    ...
    it('should support unhelpful vote type', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/forum/answers/answer-1/vote',
        { method: 'POST', body: JSON.stringify({ vote_type: 'unhelpful' }) }
      )
      const response = await votePost(request, { params: { id: 'answer-1' } })
      expect(response.status).toBe(200)
    })
    ...
    it('should validate vote_type parameter', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/forum/answers/answer-1/vote',
        { method: 'POST', body: JSON.stringify({ vote_type: 'invalid' }) }
      )
      const response = await votePost(request, { params: { id: 'answer-1' } })
  ...
  describe('DELETE /api/forum/answers/[id]/vote', () => {
    it('should remove vote with valid token', async () => {
      ...
      const response = await voteDelete(request, { params: { id: 'answer-1' } })
      expect(response.status).toBe(200)
    })
