...
import { TenureFinderResult } from '@/lib/calculators/types'
import { useToast } from '@/app/components/toast-context'
import { PaywallGate } from '@/app/components/PaywallGate'

export default function TenureFinderPage() {
  ...
  const [futureValue, setFutureValue] = useState('5000000')
  const [annualRate, setAnnualRate] = useState('10')
  const [results, setResults] = useState<TenureFinderResult | null>(null)
  ...
  const handleCalculate = async () => {
    try {
      ...
      fetch('/api/calculators/save', {
        ...
        body: JSON.stringify({
          calculator_type: 'tenure-finder',
          inputs: { presentValue, futureValue, annualRate },
          results: r,
          is_draft: true,
        }),
      }).catch(() => undefined)
      ...
      addToast('Error calculating required tenure', 'error')
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-4xl font-bold text-gray-900 mb-3">Tenure Finder</h1>
        <p className="text-gray-600 mb-8">
          How long will it take to reach your target at the given rate?
        </p>

        <PaywallGate
          requires="silver"
          reason="Tenure Finder is part of the advanced calculator suite. Upgrade to Silver to unlock all 15 calculators."
        >
        ...
        {results && (
          <div className="card p-8">
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <Stat label="Required Years" value={`${results.requiredYears} yrs`} accent />
              <Stat label="Required Months" value={`${results.requiredMonths} mo`} />
            </div>
